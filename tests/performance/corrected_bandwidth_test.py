#!/usr/bin/env python3
"""
Corrected Bandwidth Bottleneck Test

This script properly calculates efficiency metrics to test if internet
...
async def test_bandwidth_bottleneck():
    """Test bandwidth bottleneck with corrected calculations"""
    print("🌐 CORRECTED BANDWIDTH BOTTLENECK TEST")
    print("="*70)
    ...
    if result2a['success'] and result2b['success']:
        ...
        # Time efficiency: how much faster is dual vs single (should be > 100% for good scaling)
        time_speedup = single_time / dual_time
        time_efficiency = time_speedup * 100  # Percentage of ideal 2x speedup
        
        single_throughput = result1['download_throughput']
        dual_throughput = combined_download_throughput
        
        # Throughput efficiency: how close to 2x improvement we got
        throughput_speedup = dual_throughput / single_throughput if single_throughput > 0 else 0
        throughput_efficiency = (throughput_speedup / 2.0) * 100  # Percentage of ideal 2x throughput
        ...
        print(f"• Time efficiency: {time_efficiency:.1f}% (of ideal 2x)")
        ...
        print(f"• Throughput efficiency: {throughput_efficiency:.1f}% (of ideal 2x)")
        
        # Determine bottleneck with corrected logic
        ...
        # Calculate actual per-instrument performance
        single_per_instrument = single_time / result1['instruments_processed'] if result1['instruments_processed'] > 0 else 0
        dual_per_instrument = dual_time / total_instruments if total_instruments > 0 else 0
        
        print(f"\n📊 PER-INSTRUMENT PERFORMANCE:")
        print(f"• Single process: {single_per_instrument:.2f}s per instrument")
        print(f"• Dual processes: {dual_per_instrument:.2f}s per instrument")
        print(f"• Per-instrument speedup: {single_per_instrument/dual_per_instrument:.2f}x" if dual_per_instrument > 0 else "• Per-instrument speedup: N/A")
