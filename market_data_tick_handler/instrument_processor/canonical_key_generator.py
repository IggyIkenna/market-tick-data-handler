...
# from config import get_config  # Import moved to methods to avoid module-level loading
...
class CanonicalInstrumentKeyGenerator:
    ...
    def _get_tardis_data_types(self, instrument_key: str) -> List[str]:
        ...
        if "SPOT_PAIR" in instrument_key:
            ...
            # Note: liquidations and derivative_ticker may not be available for all options
            return base_data_types + ["options_chain"]
        else:
    ...
    def _extract_symbol_details(self, exchange: str, symbol_id: str, symbol_type: str, data_types: List[str]) -> Dict[str, Any]:
        ...
        if exchange == 'upbit':
            ...
            # Handle Deribit futures/options
            if symbol_id == 'OPTIONS':
                ...
                # Format: BTC-29DEC23-50000-C, BTC-29DEC23, or BTCDVOL_USDC-30AUG23
                parts = symbol_id.split('-')
                first_part = parts[0]  # BTC, ETH, SOL, BTCDVOL_USDC, etc.
                
                # Use remove_suffix to properly extract base and quote currencies
                # This handles cases like BTCDVOL_USDC where USDC is the quote currency
                base_currency, detected_quote = remove_suffix(first_part)
                
                # Clean up trailing underscores from base currency
                base_currency = base_currency.rstrip('_')
                
                # Check if we actually found a currency suffix in the symbol name
                # If the original first_part contained a currency suffix, use that currency
                if detected_quote in ['USD', 'USDC'] and detected_quote in first_part.upper():
                    # This is a properly quoted instrument (USD or USDC) - use detected currency
                    quote_currency = detected_quote
                    margin_currency = detected_quote  # USDC/USD settled
                else:
                    # Traditional coin-margined instrument (BTC, ETH, etc.)
    ...
    def save_instruments(self, instruments: Dict[str, Any], test_date: datetime, json_path: str = None, parquet_path: str = None):
        ...
        try:
            ...
            for instrument_key, details in instruments.items():
                ...
                available_from = details['available_from_datetime']
                available_to = details['available_to_datetime']
                
                # Convert to timezone-aware if not already
                if isinstance(available_from, datetime) and available_from.tzinfo is None:
                    available_from = available_from.replace(tzinfo=timezone.utc)
                if isinstance(available_to, datetime) and available_to.tzinfo is None:
                    available_to = available_to.replace(tzinfo=timezone.utc)
