...
class BookSnapshot:
    ...
    def __post_init__(self):
        ...
        for level in range(1, 6):  # Levels 1-5
            if getattr(self, f'ask_price_{level}') <= 0:
                raise ValueError(f"Ask price at level {level} must be positive")
            if getattr(self, f'bid_price_{level}') <= 0:
                raise ValueError(f"Bid price at level {level} must be positive")
            if getattr(self, f'ask_volume_{level}') <= 0:
                raise ValueError(f"Ask volume at level {level} must be positive")
            if getattr(self, f'bid_volume_{level}') <= 0:
                raise ValueError(f"Bid volume at level {level} must be positive")
...
@dataclass
class OptionsChain:
    """Options chain data model following actual Tardis API schema (exchange/symbol dropped after validation)"""
    timestamp: int  # microseconds since epoch
    local_timestamp: int  # microseconds since epoch
    type: str  # put/call
    strike_price: float
    ...
    underlying_index: str  # underlying instrument symbol
    underlying_price: float  # underlying price (was index_price in schema)
    delta: float
    gamma: float
    vega: float
    theta: float
