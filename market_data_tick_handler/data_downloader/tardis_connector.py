...
from market_data_tick_handler.models import TradeData, BookSnapshot, DerivativeTicker, Liquidations, TickData, OptionsChain
...
class TardisConnector:
    ...
    async def _create_session(self):
        ...
        if self._session is None or self._session.closed:
            ...
            headers = {
                ...
                'Keep-Alive': 'timeout=300, max=100'  # Keep alive for 5 minutes, max 100 requests
            }
            
            # Optimized connection pooling for Tardis API
            connector = aiohttp.TCPConnector(
                limit=200,                    # Total connection pool size
                limit_per_host=50,            # Connections per host (Tardis)
                keepalive_timeout=300,        # Keep connections alive for 5 minutes
                enable_cleanup_closed=True,   # Clean up closed connections
                force_close=False,            # Don't force close connections
                ssl=False                     # Tardis uses HTTPS but we'll let aiohttp handle it
            )
    ...
    async def _make_request(self, url: str, params: Dict[str, Any] = None) -> TardisResponse:
        ...
        # Safety check - ensure session is created
    ...
    def _type_book_snapshot_dataframe(self, df: pd.DataFrame) -> pd.DataFrame:
        """Apply proper typing to book snapshot DataFrame and convert to BigQuery-compatible format"""
        # Convert timestamps to int64 (microseconds) - handle NaN values
        df['timestamp'] = pd.to_numeric(df['timestamp'], errors='coerce').astype('Int64')
        df['local_timestamp'] = pd.to_numeric(df['local_timestamp'], errors='coerce').astype('Int64')
        
        # Convert Tardis format (asks[0].price) to BigQuery-compatible format (ask_price_1)
        for i in range(5):  # Level 5 book
            level = i + 1  # Convert 0-based to 1-based indexing for BigQuery compatibility
    ...
    def _type_options_chain_dataframe(self, df: pd.DataFrame) -> pd.DataFrame:
        """Apply proper typing to options chain DataFrame according to actual Tardis API schema"""
        # Keep exchange and symbol as strings for validation (if present)
        if 'exchange' in df.columns:
            df['exchange'] = df['exchange'].astype('string')
        if 'symbol' in df.columns:
            df['symbol'] = df['symbol'].astype('string')
        ...
        # Convert numeric fields to float64 according to actual Tardis API schema
        numeric_fields = [
            'strike_price', 'open_interest', 'last_price', 'bid_price', 'bid_amount', 
    ...
    def _parse_book_snapshot_data(self, records: List[Dict[str, Any]]) -> List[BookSnapshot]:
        """Parse book snapshot data into BookSnapshot objects using BigQuery-compatible format"""
        snapshots = []
        ...
        for record in records:
            try:
                ...
                for i in range(5):  # Level 5 book
                    ...
                    snapshot_data[f'bid_price_{level}'] = float(record.get(bid_price_key, 0))
                    snapshot_data[f'bid_volume_{level}'] = float(record.get(bid_amount_key, 0))
                    snapshot_data[f'ask_price_{level}'] = float(record.get(ask_price_key, 0))
                    snapshot_data[f'ask_volume_{level}'] = float(record.get(ask_amount_key, 0))
                
                snapshot = BookSnapshot.from_datetimes(**snapshot_data)
    ...
    async def download_daily_data_direct(self, tardis_exchange: str, tardis_symbol: str, 
        ...
        for data_type in data_types:
            ...
            try:
                ...
                result[data_type] = None  # None indicates failure (not empty list)
    ...
    def _create_empty_dataframe_with_schema(self, data_type: str) -> pd.DataFrame:
        ...
        if data_type == 'trades':
            ...
            # Create empty DataFrame with BigQuery-compatible BookSnapshot schema
            df = pd.DataFrame(columns=[
                ...
                'ask_price_5', 'ask_volume_5', 'bid_price_5', 'bid_volume_5'
            ])
            # Set proper dtypes
            df['timestamp'] = df['timestamp'].astype('Int64')
            df['local_timestamp'] = df['local_timestamp'].astype('Int64')
    ...
    def _validate_tardis_data(self, df: pd.DataFrame, expected_exchange: str, expected_symbol: str, data_type: str):
        ...
        if 'exchange' in df.columns and not df.empty:
            ...
            elif actual_exchanges[0].lower() != expected_exchange.lower():
        ...
        if 'symbol' in df.columns and not df.empty:
            ...
            if len(actual_symbols) > 1:
                ...
                actual_symbol = actual_symbols[0]
                # Case-insensitive comparison for symbol validation
                if actual_symbol.lower() != expected_symbol.lower():
                    # Check if it's just a case difference (common with Tardis)
                    if actual_symbol.upper() == expected_symbol.upper():
                        logger.debug(f"Symbol case difference in {data_type}: expected {expected_symbol}, got {actual_symbol} (acceptable)")
                    else:
                        logger.warning(f"Symbol mismatch in {data_type}: expected {expected_symbol}, got {actual_symbol}")
    ...
    def save_to_parquet(self, df: pd.DataFrame, output_path: Path) -> None:
        """Save DataFrame to Parquet format with optimization for sparse data access"""
        try:
            ...
            # Save as optimized Parquet with compression and indexing
            df.to_parquet(
                output_path,
                engine='pyarrow',
                compression='snappy',
                index=False,
                # Optimization settings for sparse data access
                ...
                column_encoding={
                    'local_timestamp': 'DELTA_BINARY_PACKED',  # Primary query column for tick data
                    'timestamp': 'DELTA_BINARY_PACKED'         # Secondary timestamp
                }
            )
            logger.info(f"Saved {len(df)} records to optimized Parquet: {output_path}")
