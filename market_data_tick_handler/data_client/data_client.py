"""
Data Client for GCS Data Retrieval

Simplified data access client for downstream services to read:
- Instrument definitions
- Tick data with timestamp filtering
- Candle data across timeframes
- Feature data (HFT/MFT)

Removes REST API specific logic and focuses on clean data access.
"""
...
class DataClient:
    """Clean data access client for GCS data retrieval"""
    ...
    def _initialize_client(self):
        ...
        try:
            if self.config.auth.use_mock_data:
                logger.info("Using mock data client for offline development")
                from .mock_data_client import MockDataClient
                self._mock_client = MockDataClient(self.config.auth.mock_data_path)
                self._is_mock = True
                return
            
            # Try to initialize GCS client
            self.client = get_shared_gcs_client()
            self.bucket = get_shared_gcs_bucket(self.gcs_bucket)
            self._is_mock = False
            
        except Exception as e:
            if self.config.auth.use_mock_data:
                logger.info("GCS unavailable, falling back to mock data")
                from .mock_data_client import MockDataClient
                self._mock_client = MockDataClient(self.config.auth.mock_data_path)
                self._is_mock = True
            else:
    ...
    def is_mock(self) -> bool:
        """Check if using mock data client"""
    ...
    async def get_instrument_definitions(
        ...
        """
        context = ErrorContext(operation="get_instrument_definitions", component="data_client")
    ...
    async def get_tick_data(
        ...
        max_instruments_per_query: int = 1000
    ) -> Dict[str, Any]:
        """
        ...
        Args:
            ...
            max_date_range_days: Maximum date range in days (default: 365)
            max_instruments_per_query: Maximum instruments per query (default: 1000)
            
        Returns:
            Dict containing file paths organized by date and data type
        """
        context = ErrorContext(operation="get_tick_data", component="data_client")
        ...
        try:
            ...
            if len(instrument_ids) > max_instruments_per_query:
                raise ValueError(f"Too many instruments requested. Maximum: {max_instruments_per_query}")
    ...
    async def get_tick_data_paths(
        ...
        """
        Get file paths for tick data from GCS (simplified version without validation)
        ...
        """
        context = ErrorContext(operation="get_tick_data_paths", component="data_client")
        log_operation_start(logger, "get_tick_data_paths", 
            instrument_ids=instrument_ids[:5],  # Log first 5 for brevity
            start_date=start_date,
            end_date=end_date,
            data_types=data_types
        ...
        try:
            ...
            log_operation_success(logger, "get_tick_data_paths", files_found=len(file_paths))
            
            return file_paths
            
        except Exception as e:
            log_operation_failure(logger, "get_tick_data_paths", error=str(e))
            enhanced_error = self.error_handler.handle_error(e, context)
            raise enhanced_error
    ...
    def generate_signed_url(self, blob_name: str, expiry_hours: int = 168) -> str:
