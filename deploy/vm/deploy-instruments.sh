...
# VM Deployment script for Instrument Definitions
# This script deploys and manages VMs for generating instrument definitions
...
show_usage() {
    echo -e "${BLUE}VM Deployment for Instrument Definitions${NC}"
    echo "============================================="
    echo ""
    echo "Usage: $0 <command> [options]"
    echo ""
    echo "Commands:"
    echo "  deploy          - Deploy a new VM for instrument generation"
...
deploy_vm() {
    echo -e "${BLUE}☁️  Deploying Instrument Generation VM${NC}"
    echo "============================================="
...
EOF
    ...
    gcloud compute instances create "$VM_NAME" \
        ...
        --metadata-from-file startup-script=/tmp/instrument-startup.sh \
        --scopes=https://www.googleapis.com/auth/cloud-platform \
        --tags=instrument-generator
    ...
    echo "3. Run instrument generation: $0 run --start-date 2023-05-23 --end-date 2023-05-25"

    # Clean up
    rm -f /tmp/instrument-startup.sh
}
...
show_status() {
    ...
    if check_vm_exists; then
        ...
        if [ "$VM_STATUS" = "RUNNING" ]; then
            ...
            # Check if instrument generation is running
            echo -e "\n${YELLOW}Checking instrument generation process...${NC}"
            gcloud compute ssh "$VM_NAME" --zone="$ZONE" --project="$PROJECT_ID" --command="
                if pgrep -f 'python.*main.*instruments' > /dev/null; then
                    echo '✅ Instrument generation is running'
                    ps aux | grep 'python.*main.*instruments' | grep -v grep
                else
                    echo '❌ Instrument generation is not running'
                fi
...
view_logs() {
    ...
    if check_vm_exists; then
        ...
        if [ "$VM_STATUS" = "RUNNING" ]; then
            gcloud compute ssh "$VM_NAME" --zone="$ZONE" --project="$PROJECT_ID" --command="
                cd /opt/market-tick-data-handler
                if [ -f logs/instrument_generation.log ]; then
                    tail -50 logs/instrument_generation.log
                else
                    echo 'No logs found'
                fi
            "
...
# Function to run instrument generation
run_instruments() {
    local start_date=""
    local end_date=""
    local venues=""
    ...
    echo -e "${BLUE}🏃 Running instrument generation...${NC}"
